...
class ColorTextRenderer(StyleTextRenderer):
    ...
    def __init__(self, config_file=None, escape_code_prefixes=None, escape_code_colors=None, escape_code_styles=None,
                 color_theme: str = None, color_groups=None):
        super().__init__(config_file)

        self.escape_code_prefixes = escape_code_prefixes or self.config.escape_codes.prefixes
        self.escape_code_colors = escape_code_colors or self.config.escape_codes.colors['256']
    ...
    def wrap_escape_code(self, code: str | list[str]) -> str:
        ...
        return self.enc(str(self.escape_code_prefixes.escape) + "".join(code) + str(self.escape_code_prefixes.end))

    def color(self, color_name: str, background_color: bool = False):
        return self.wrap_escape_code([
            self.escape_code_prefixes.background if background_color else self.escape_code_prefixes.foreground,
    ...
    def get_color_code(self, color: str) -> str | None:
        ...
        hex_color_list = [c for c in list(self.escape_code_colors['hex'].keys())
